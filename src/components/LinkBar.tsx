...
function LinkBar({ link, className }: LinkBarProps) {
  ...
  return (
    <button
      className={`group flex w-full items-center border-[1.5px] ${
        isCopied ? 'border-slate-400' : 'border-secondary'
      ...
      <p className='overflow-x-scroll text-nowrap no-scrollbar px-2 text-slate-500 cursor-pointer'>
        {link}
      </p>
    </button>
  )
