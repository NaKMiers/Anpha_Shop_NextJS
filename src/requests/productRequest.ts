...
export const getSuggestedProductsApi = async (
  ...
  // no-store to avoid cache
  const res = await fetch(`/api/product/suggest${query}`, option)
...
// [PATCH]
export const bootProductsApi = async (ids: string[], value: boolean) => {
  const res = await fetch('/api/admin/product/boot', {
    method: 'PATCH',
    body: JSON.stringify({ ids, value }),
  })
