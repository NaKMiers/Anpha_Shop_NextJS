...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const products = await ProductModel.find({ active: true, category: { $in: categoryIds }, ...filter })
    ...
    const amount = await ProductModel.countDocuments({ category: { $in: categoryIds }, ...filter })
